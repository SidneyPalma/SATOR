...
class Helper extends \Smart\Data\Proxy
{
    ...
    public function selectFields() {
        ...
        $sql = "SELECT ORDINAL_POSITION, COLUMN_NAME, COLUMN_DEFAULT, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH, IS_NULLABLE, 1 as HAS_POLICY, 0 as HAS_IGNORE, 1 as HAS_LOGALLOW FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = :TABLE_SCHEMA AND TABLE_NAME = :TABLE_NAME";
    ...
    private function createExtJS() {
        ...
        if (!file_exists($root . "store/" . implode("/", $list). "/")) {
            mkdir($root . "store/" . implode("/", $list). "/", 0777,true);
            chmod($root . "store/" . implode("/", $list). "/", 0777);
        }
        ...
        fwrite($file, "    url: '../$namespace/business/Calls/$storeId.php',");
        fwrite($file, "\r\n\r\n");
        ...
        if($datapackage == 'true') {
            if (!file_exists($root . "model/" . implode("/", $list). "/")) {
                mkdir($root . "model/" . implode("/", $list). "/", 0777,true);
                chmod($root . "model/" . implode("/", $list). "/", 0777);
            }
    ...
    private function createModel() {
        ...
        fwrite($file, "/**\r\n * \r\n * ");
        fwrite($file, '@Entity {"name":"'.$classname.'", "logbook":true, "cache":"' . $cache. '", "event":"' . $event . '"}');
    ...
    private function createEvent() {
        ...
        fwrite($file, "\r\n\r\n");
        fwrite($file, "use Smart\\Utils\\Session;");
        fwrite($file, "\r\n\r\n");
        ...
        fwrite($file, "        Session::hasProfile('','');");
        fwrite($file, "\r\n");
        fwrite($file, "    }\r\n\r\n");
        fwrite($file, "    /**\r\n");
        fwrite($file, "     * @param \\{$namespace}\\Model\\{$classname}" . ' $model' . "\r\n");
    ...
    private function createCoach() {
        ...
        fwrite($file, "\r\n\r\n");
        fwrite($file, "    public function select() {");
        fwrite($file, "\r\n");
        fwrite($file, "        try {");
        fwrite($file, "\r\n");
        fwrite($file, "            Session::hasProfile('','');");
        fwrite($file, "\r\n");
        fwrite($file, '            $result'." = parent::select();");
        fwrite($file, "\r\n");
        fwrite($file, "        } catch ( \\PDOException ".'$e'." ) {");
        fwrite($file, "\r\n");
        fwrite($file, "            self::_setSuccess(false);");
        fwrite($file, "\r\n");
        fwrite($file, "            self::_setText(".'$e'."->getMessage());");
        fwrite($file, "\r\n");
        fwrite($file, '            $result'." = self::getResultToJson();");
        fwrite($file, "\r\n");
        fwrite($file, "        }");
        fwrite($file, "\r\n\r\n");
    ...
    private function docColumn($value) {
        $docDescription = '     * @Column {"description":"'.self::_COMMENT_DESCRIPTION($value).'", '. self::_STRATEGY_TYPE($value) .'"type":"'.self::_DATA_TYPE($value).'", "policy":'.self::_HAS_POLICY($value) . ', "logallow":'.self::_HAS_LOGALLOW($value). self::_COLUMN_DEFAULT($value) .'}';
    ...
    private static function _HAS_IGNORE (array $array) {
        ...
        return ($value == '0' || $value == 'false' || strlen($value) == 0 ) ? 'false' : 'true';
    }
    ...
    private static function _HAS_LOGALLOW (array $array) {
        $value = $array["HAS_LOGALLOW"];
    ...
    private static function _CHARACTER_MAXIMUM_LENGTH (array $array) {
        ...
        return $value ? ', "length":' . ($value == '-1' ? '0' : $value) : '';
    }
